  ...
  useEffect(() => {
    ...
    properties.forEach(p => {
      ...
      const popup = new maplibregl.Popup({ closeButton: false, closeOnClick: false, offset: 18, maxWidth: '280px' })
        .setHTML(`<div style="font-family:Inter,sans-serif;cursor:pointer;" onclick="window.location.hash='#/property/${p.slug}'">
...
export default function SearchPage() {
  ...
  const [mapVisible, _setMapVisible] = useState(true)
  ...
  return (
    <div className="min-h-[100dvh] bg-cream">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between py-3 gap-2">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <div className="flex border border-border-warm rounded-lg overflow-hidden">
                ...
                <button onClick={() => setView(view === 'map' ? 'grid' : 'map')} className={cn('hidden lg:flex w-9 h-9 items-center justify-center', view === 'map' ? 'bg-terracotta text-white' : 'bg-white text-text-secondary hover:bg-cream')} title="Carte">
                  <Map size={16} />
                </button>
              </div>
      ...
      <div className="max-w-[1600px] mx-auto flex">
        ...
        {/* ── DESKTOP: Mode Carte layout ── */}
        {view === 'map' && (
          <div className="hidden lg:flex flex-1 min-h-[calc(100dvh-152px)]">
            <aside className="w-[380px] flex-shrink-0 border-r border-border-warm bg-cream flex flex-col">
              <div className="px-4 py-3 border-b border-border-warm bg-white">
                <p className="text-midnight font-inter text-[14px] font-semibold">{filtered.length} bien{filtered.length > 1 ? 's' : ''}</p>
                <p className="text-text-secondary text-[12px]">À {filters.transaction === 'sale' ? 'vendre' : 'louer'} à Marrakech</p>
              </div>
        ...
        {/* ── NORMAL layout (grid/list + map panel) ── */}
        {view !== 'map' && (
          <>
